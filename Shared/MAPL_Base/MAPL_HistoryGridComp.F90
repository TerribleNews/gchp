...
! --------------------------------------------------
    ...
    call ESMF_ClockGet ( clock,     stopTime=EndTime,    rc=STATUS ) ; _VERIFY(STATUS)
...
!       endif
    ...
    do n=1,nlist
       ...
       if (list(n)%format == 'CFIO' .or. list(n)%format == 'CFIOasync') then
          ...
          list(n)%mNewCFIO%collection_id = o_ClientPtr%add_hist_collection(list(n)%mNewCFIO%metadata)
       end if
   end do
      ...
      if( Writing(n) ) then
         ...
         if( list(n)%unit.eq.0 ) then
            if (list(n)%format == 'CFIO') then
               call list(n)%mNewCFIO%modifyTime(rc=status)
               _VERIFY(status) 
               list(n)%unit = -1
            else
...
!
   ...
   POSTLOOP: do n=1,nlist
      ...
      OUTTIME: if( Writing(n) ) then
         ...
         IOTYPE: if (list(n)%unit < 0) then    ! CFIO
            ...
            call list(n)%mNewCFIO%bundlepost(filename(n),rc=status)
            _VERIFY(status)
   ...
   WAITLOOP: do n=1,nlist
      ...
      if( Writing(n) .and. list(n)%unit < 0) then
         ...
         call list(n)%mNewCFIO%bundlewait(rc=status)
         _VERIFY(status)
