...
!!$    integer, intent(in) :: mapl_comm
  ...
  subroutine run_MAPL_GridComp(gc, rc)
    ...
    type(ESMF_Time)          :: stopTime, currTime
      ...
      type(ESMF_Alarm) :: alarm(200), hist_alarm(200)
...
1000 format(1x,'AGCM Date: ',i4.4,'/',i2.2,'/',i2.2,2x,'Time: ',i2.2,':',i2.2,':',i2.2)
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! ! MAT This is Bill's new per-step timers which are incompatible with develop                                                                      !
   ! !     Not sure how to fix                                                                                                                         !
   !                                                                                                                                                   !
   ...
   !    real(kind=REAL64) :: START_RUN_TIMER,END_RUN_TIMER,START_TIMER,END_TIMER,CAP_TIMER,LOOP_TIMER                                                  !
   !    real(kind=REAL64) :: TIME_REMAINING                                                                                                            !
   !    real(kind=REAL64) ::   CAP_THROUGHPUT=0.0_REAL64                                                                                               !
   ...
   !        RUN_THROUGHPUT = REAL(  HEARTBEAT_DT,kind=REAL64)/(END_RUN_TIMER-START_RUN_TIMER)                                                          !
   !     ! Time loop throughput [Inst]                                                                                                                 !
   !       INST_THROUGHPUT = REAL(  HEARTBEAT_DT,kind=REAL64)/(END_TIMER-START_TIMER)                                                                  !
   !     ! Time loop throughput [Avg]                                                                                                                  !
   !       LOOP_THROUGHPUT = REAL(n*HEARTBEAT_DT,kind=REAL64)/(END_TIMER- LOOP_TIMER)                                                                  !
   !     ! CAP throughput [Avg]                                                                                                                        !
   !        CAP_THROUGHPUT = REAL(n*HEARTBEAT_DT,kind=REAL64)/(END_TIMER-  CAP_TIMER)                                                                  !
   !     ! Estimate time remaining (seconds)                                                                                                           !
   !       TIME_REMAINING = REAL((nsteps-n)*HEARTBEAT_DT,kind=REAL64)/LOOP_THROUGHPUT                                                                  !
